...
<<<<<<< HEAD
  // Sistema de atualização do Service Worker
...
=======
  // Removidas as referências para navegação por swipe que estavam causando problemas
  const mainRef = useRef<HTMLDivElement>(null);

>>>>>>> 622891e55def6135763b098669e03a4fab073923
  ...
  // ⭐⭐ MUDANÇA 1: Filtrar clientes - saldo zero NÃO aparece na lista, MAS aparece se pesquisado
  const filteredClients = useMemo(() => {
    if (!searchQuery) {
      // Lista normal: mostra apenas clientes com saldo ≠ 0
      return clients.filter(c => {
        const balance = getClientBalance(c);
        return balance !== 0;
      });
    } else {
      // Busca: mostra TODOS os clientes que correspondem à pesquisa
      return clients.filter(c => 
...
<<<<<<< HEAD
        // ⭐⭐ MUDANÇA 2: Removidos todos os handlers de swipe
=======
        // Removidos os handlers de touch que causavam problemas
>>>>>>> 622891e55def6135763b098669e03a4fab073923
      >
        <main 
            ...
            <div className="p-6 pb-24 space-y-6 animate-in fade-in duration-500">
              ...
              <div className="grid gap-3">
                {filteredClients.length > 0 ? (
                  ...
                  // ⭐⭐ MUDANÇA 3: Mensagem quando não há clientes com saldo
                  <div className="py-16 text-center opacity-50">
                    <Users className="w-16 h-16 mx-auto mb-4 text-slate-400" />
                    <p className="font-bold text-lg mb-2">Nenhum cliente com saldo</p>
                    <p className="text-sm text-slate-500">Todos os clientes estão com saldo zerado.</p>
                    <p className="text-xs text-slate-400 mt-2">Use a busca para encontrar clientes arquivados.</p>
                  </div>
                )}
              </div>
        ...
        {showUpdatePrompt && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className={`${isDark ? 'bg-slate-900' : 'bg-white'} m-4 rounded-3xl p-8 max-w-sm w-full shadow-2xl`}>
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 rounded-2xl bg-blue-500/20 flex items-center justify-center">
                  <Cloud className="w-6 h-6 text-blue-500" />
                </div>
                <div>
                  <h3 className="text-xl font-black">Nova Versão!</h3>
                  <p className="text-sm text-gray-500">Atualização disponível</p>
                </div>
              </div>
              
              <p className="mb-6 text-gray-600 dark:text-gray-300">
                Uma nova versão do app está disponível com melhorias de desempenho e correções.
              </p>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 px-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg active:scale-95"
                >
                  Atualizar
                </button>
              </div>
